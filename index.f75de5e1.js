...
for (const currTh of thCollection)currTh.setAttribute("data-state", "unsorted");
...
tbody.addEventListener("click", function(ev) {
    ...
    if (tr) {
        const tbodyRows = document.querySelectorAll("tbody tr");
        for (const row of tbodyRows)if (row.classList.contains("active")) row.classList.remove("active");
...
button.addEventListener("click", function(ev) {
    ...
    if (nameLengthError || nameSpacesError || positionLengthError || positionSpacesError || officeSpacesError || ageLimitsError || ageSpacesError || salarySpacesError) {
